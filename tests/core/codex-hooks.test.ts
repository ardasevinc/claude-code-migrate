...
`;
  ...
  it("strips verified trust while preserving enabled state when requested", async () => {
    const hooks = JSON.stringify({
      hooks: {
        PreToolUse: [
          {
...
`;
    ...
    expect(adapted.configContent).toContain("enabled = false");
    expect(adapted.configContent).not.toContain("trusted_hash");
  });

  it("removes duplicate hook states and prefers the exact target enabled state", async () => {
    const hooks = JSON.stringify({
      hooks: {
        PreToolUse: [
          {
...
`;
    ...
    expect(adapted.trusted).toBe(0);
    expect(adapted.configContent.match(/\[hooks\.state\./g)).toHaveLength(1);
    expect(adapted.configContent).not.toContain("/Users/arda/.codex/hooks.json");
    expect(adapted.configContent).toContain("enabled = true");
    expect(adapted.configContent).not.toContain("trusted_hash");
  });
});
